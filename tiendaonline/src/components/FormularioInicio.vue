    ...
    <q-card class="q-pa-md" style="width: 60%; left: 18%;">
      <q-card-section>
        <q-form @submit.prevent="iniciarSesion" @reset="resetFormulario">
          ...
          <div class="q-mt-md">
            <q-btn label="Iniciar Sesión" type="submit" class="botones" :loading="cargando" />
      ...
      <q-card-section class="q-pa-sm" style="background-color: #f0f0f0; font-size: large;">
        <p class="q-mb-none">¿Todavía no tienes cuenta?</p>
        <router-link to="/registro"><strong style="color: red; text-decoration: underline;">Crea una aquí</strong></router-link>
      </q-card-section>
    </q-card>
  </section>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const iniciarSesion = () => {
      ...
      axios
        .post("http://localhost:3900/api/inicioSesion", {
          correo: correo.value,
        ...
        .then((response) => {
          ...
          router.push("/home").then(() => {
            window.location.reload(); // Recargar la página después del redireccionamiento esto es para detectar el Token
          });
        })
        .catch((err) => {
          cargando.value = false;
          mostrarAlerta.value = false;
          mensajeError.value = err.response.data.mensaje || "Error al iniciar sesión";
          mostrarError.value = true;
          console.error(err);
        });
    };
